...
8. **`applenews://search?term=` is not a real URL** — it was tried and reverted; the scheme launches the News app but has no search path, and feed readers drop non-`http(s)` links entirely. Only `https://apple.news/A…` works, and its ID must be discovered, never constructed.
